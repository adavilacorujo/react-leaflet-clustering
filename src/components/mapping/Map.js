...
const createNetworkMarkers = (nodes, markers) => {
  // Filter out nodes
  const tempResults = new Map(
    [...nodes].filter(([, v]) => v["primary"] === false)
  );
  // Populate array
  tempResults.forEach((value) => {
    if (value["type"] === "sub") {
...
const createPolylines = (searchResults, selectedDataPoints) => {
  ...
  if (selectedDataPoints.length < 1) {
    ...
    links.forEach((link) => {
      ...
      polylines.push({
        polyline: (
          <Polyline key={source["id"] + target["id"]} positions={polyline} />
        ),
        show: true,
        source: source["id"],
        target: target["id"],
    ...
    links.forEach((link) => {
      ...
      polylines.push({
        ...
        show:
          selectedDataPoints.includes(source["id"]) &&
          selectedDataPoints.includes(target["id"]),
        source: source["id"],
        target: target["id"],
      });
    });
...
const MapView = ({
  ...
  useEffect(() => {
    if (searchResults?.nodes !== undefined) {
      ...
      if (showPolyLines)
        setPolylines(createPolylines(searchResults, selectedDataPoints));
      else setPolylines([]);
  ...
  useEffect(() => {
    if (showRelationships) {
      ...
      const tempIdList = markers
        .filter((marker) => marker["primary"] === false)
        .map((element) => element["id"]);
      setSelectedDataPoints([...selectedDataPoints, ...tempIdList]);
    } else {
      // only keep primary search results
      const tempIdList = markers
        .filter((marker) => marker["primary"] === true)
        .map((element) => element["id"]);
