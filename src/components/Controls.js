...
export const Controls = ({switches, setSwitches, selectedDataPoints, setSelectedDataPoints, searchResults}) => {
    ...
    const handleClick = (event) => {
        ...
        const found = selectedDataPoints.some(element => element === dataPoint.id)
        if (!found) {
    ...
    return (
        ...
        <FormControl>
          <FormLabel component="legend">Control Markers</FormLabel>
            ...
            <Tooltip title="Switch to show markers with relationships">
                <FormControlLabel
                    control={
                        <Switch checked={switches.showRelationships} onChange={handleChange} name="showRelationships" />
                    }
            ...
            <Tooltip title="Switch to show connections between markers">
                 <FormControlLabel
                    control={
                        <Switch checked={switches.showPolyLines} onChange={handleChange} name="showPolyLines" 
        ...
        <Tooltip title="Click to show returned data points and reset switches">
            <Button variant="outlined" sx={{width: '100%'}} onClick={handlFreshButton}>Freshen Up</Button>
        </Tooltip>
        <Divider sx={{margin: '1rem'}}/>
        <div style={{flexFlow: 'column nowrap'}}>
        Data points
        <Stack direction="column" justifyContent='space-evenly' spacing={2}>
        {
            searchData.map(value => (
                <Button sx={{
                    borderRadius: '2rem', border: '1px dashed blue',
                    fontSize: '10px',
                    backgroundColor: value['selected'] ? `rgb(153, 204, 255)` : null,
                    color: value['selected'] ? 'white' : null,
                    fontWeight: value['selected'] ? 'bold' : 'normal',
                }} onClick={handleClick} size="small" variant="elevated" value={value['id']}>
                {value['name']}
                </Button>
            ))
        }
